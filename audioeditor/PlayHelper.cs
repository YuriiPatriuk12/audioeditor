...
namespace Audioeditor
{
    public class PlayHelper
    {
        ...
        public (float[], float[]) GetAudioDataBatch(int startSample, int batchSize)
        {
            ...
            if (leftAudioDataBatch.Length == 0 || rightAudioDataBatch.Length == 0)
            {
                MessageBox.Show("Audio data batch is empty.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                ...
                if (!channel.IsMuted)
                {
                    ...
                    if ((isSomeSoloed && channel.IsSoloed) || !isSomeSoloed)
                    {
                        ...
                        if (channel.Plugins != null)
                        {
                            ...
                            {
                                leftChannelAudioDataBatch = EqualizeAudio(leftChannelAudioDataBatch, plugin.Channel.LowFreq,
                                    plugin.Channel.MidFreq, plugin.Channel.HighFreq);
                                rightChannelAudioDataBatch = EqualizeAudio(rightChannelAudioDataBatch, plugin.Channel.LowFreq,
                                    plugin.Channel.MidFreq, plugin.Channel.HighFreq);
                            }
                        }
        ...
        public float[] EqualizeAudio(float[] audioData, float lowGain, float midGain, float highGain)
        {
            ...
            {
                audioData = audioData.Take(fftSize).ToArray(); 
            }
            ...
            for (int i = 0; i < equalizedAudioData.Length; i++)
            {
            ...
            for (int i = 0; i < equalizedAudioData.Length; i++)
            {
                equalizedAudioData[i] = Math.Max(-0.8f, Math.Min(equalizedAudioData[i], 0.8f));
            }
        ...
        public (float[], float[]) GetFullAudioData()
        {
            ...
            {
                MessageBox.Show("Audio data is empty after batch generation.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
            }
        ...
        public AudioData LoadAudioDataWithCheck(string filepath)
        {
            try
            {
                return LoadAudioData(filepath);
            }
            catch (COMException ex)
            {
                MessageBox.Show("Audio file upload error: stream type not supported.", "Error", MessageBoxButton.OK, MessageBoxImage.Error);
                return null;
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Error has occurred: {ex.Message}", "Unexpected error", MessageBoxButton.OK, MessageBoxImage.Error);
                return null;
            }
        }
